...
google.admin = ({ version }) => {
  ...
  return {
    users: { list: async () => ({ data: { users: [
      { primaryEmail: 'admin@rcs.co.za', isAdmin: true,  isEnrolledIn2Sv: true,  isEnforcedIn2Sv: true,  lastLoginTime: new Date().toISOString() },
...
// ---- Collaboration ----
ok(byId['g-groups'].status === 'partial' || byId['g-groups'].status === 'fail', 'public group flagged (' + byId['g-groups'].status + ')');
...
ok(html.includes('Fix setting'), 'dashboard has Fix setting buttons');
...
ok(html.includes('Previous:'), 'dashboard shows score trend from history');
...
const findings2 = scan.findings.map((f) => f.id === 'g-2sv' ? { ...f, status: 'fail' } : f.id === 'g-groups' ? { ...f, status: 'pass' } : f);
...
ok(drift.fixes.some((f) => f.id === 'g-groups'), 'drift: group fix (partial->pass) detected');
...
ok(org.includes('Link &amp; scan a tenant'), 'org view: has link-tenant action');
ok(org.includes('Organisation view'), 'org view: header present');
