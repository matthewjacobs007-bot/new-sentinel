...
export async function initSchema() {
  ...
  // Cancelling a Paystack subscription needs both its code and its email_token —
  // both only arrive via the subscription.create webhook event, after activation.
  await p.query(`ALTER TABLE tenants ADD COLUMN IF NOT EXISTS payment_subscription_token TEXT`);
  await p.query(`ALTER TABLE tenants ADD COLUMN IF NOT EXISTS activated_at TIMESTAMPTZ`);
  await p.query(`ALTER TABLE tenants ADD COLUMN IF NOT EXISTS cancelled_at TIMESTAMPTZ`);
