...
.filterbar{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:16px 20px;margin-bottom:14px;position:sticky;top:8px;z-index:10;backdrop-filter:blur(8px)}
...
.filterbar .lbl{font-size:10.5px;text-transform:uppercase;letter-spacing:.6px;color:var(--ink-mute);font-weight:600;margin-right:4px;min-width:64px}
.chip{display:inline-flex;align-items:center;gap:5px;padding:6px 11px;border-radius:20px;font-size:12px;font-weight:500;border:1px solid var(--line2);background:transparent;color:var(--ink-dim);cursor:pointer;transition:all .12s;font-family:inherit}
...
.searchbox{flex:1;min-width:180px;background:var(--bg2);border:1px solid var(--line2);border-radius:8px;padding:8px 12px;color:var(--ink);outline:none;font-size:13px}
.searchbox:focus{border-color:var(--cyan)}
.summary{font-size:12.5px;color:var(--ink-dim);padding:2px 4px}
.summary b{color:var(--cyan);font-weight:600}
...
export function dashboardPage(scan, accepted, baseUrl, history = [], drift = null) {
  ...
  const moduleBlocks = modules.map((m) => {
    const items = m.items.map((f) => {
      ...
      const actions = (f.status === 'fail' || f.status === 'partial' || f.status === 'unknown')
        ...
        : '';
      const statusKey = f.accepted ? 'accepted' : f.status;
      return `<div class="finding" data-status="${statusKey}" data-severity="${f.severity}" data-module="${esc(f.module)}" data-text="${esc((f.title + ' ' + f.detail).toLowerCase())}" style="padding:15px 0;border-bottom:1px solid var(--line)">
  ...
  return `<!doctype html>${HEAD}<title>${esc(tenant)} — Sentinel</title><body><div class="wrap">
    ...
    ${filterBar(modules, sev, scan.findings, accepted)}

    ${moduleBlocks}
    <div class="no-results hidden" id="noResults">No findings match the current filters. <a href="#" onclick="clearFilters();return false" style="color:var(--cyan)">Clear filters</a></div>
...
function filterBar(modules, sev, findings, accepted) {
  ...
  const modChip = (m) => `<button class="chip" data-filter="module" data-value="${esc(m.name)}" onclick="toggleFilter(this)">${esc(m.name)} <span class="n">${m.total}</span></button>`;
  ...
  return `<div class="filterbar" id="filterbar">
    <div class="row">
      ...
      <button class="chip" data-preset="quickwins" onclick="applyPreset(this,'quickwins')">⚡ Quick wins (low/medium)</button>
      <button class="chip" data-preset="review" onclick="applyPreset(this,'review')">👁 Needs manual review</button>
      <button class="chip" data-preset="healthy" onclick="applyPreset(this,'healthy')">✓ Passing controls</button>
...
function filterScript() {
  return `<script>
    const active = { status: new Set(), severity: new Set(), module: new Set() };
    ...
    function applyPreset(btn, kind) {
      ...
      else if (kind === 'healthy') { set('status', ['pass']); }
      applyFilters();
    }
    ...
    function applyFilters() {
      ...
      let shown = 0;
      findings.forEach(el => {
        const s = el.dataset.status, sv = el.dataset.severity, mo = el.dataset.module, tx = el.dataset.text;
        let ok = true;
      ...
      const total = findings.length;
      document.getElementById('summary').innerHTML = 'Showing <b>' + shown + '</b> of <b>' + total + '</b> findings';
