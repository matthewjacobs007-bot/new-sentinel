...
import { analyseEmailSecurity } from './emailSecurity.js';
import { scanActivity } from './activityScan.js';
...
export async function scanGoogle(auth) {
  ...
  // Recovery info across the whole active population (g-admin-recovery above only covers admins).
  const activeWithRecovery = active.filter((u) => u.recoveryEmail || u.recoveryPhone).length;
  const recoveryPct = active.length ? Math.round((activeWithRecovery / active.length) * 100) : 0;
  add({ id: 'g-user-recovery', module: 'Account Hygiene', severity: 'medium', cis: 'CIS 5.2',
    title: 'Recovery information set for all active users', status: pctBand(recoveryPct),
    detail: `${activeWithRecovery} of ${active.length} active users (${recoveryPct}%) have a recovery email or phone set.`,
    recommendation: 'Require recovery email/phone at provisioning to prevent lockout and speed up account-takeover response.', fixUrl: `${AC}/users` });

  // Offboarding hygiene: admin privileges should be revoked before/at suspension, not left dangling.
  const suspendedAdmins = suspended.filter((u) => u.isAdmin || u.isDelegatedAdmin);
  add({ id: 'g-suspended-admin', module: 'Account Hygiene', severity: 'critical', cis: 'CIS 5.4',
    title: 'No admin privileges left on suspended accounts', status: suspendedAdmins.length === 0 ? 'pass' : 'fail',
    detail: suspendedAdmins.length
      ? `${suspendedAdmins.length} suspended account(s) still hold admin privileges: ${suspendedAdmins.map((a) => a.primaryEmail).join(', ')}.`
      : 'No suspended account retains admin/delegated-admin privileges.',
    recommendation: 'Strip admin roles as part of the offboarding step, before or at the moment of suspension.', fixUrl: `${AC}/roles` });
  ...
  for (const u of sample) {
    ...
    for (const t of res?.data?.items || []) {
      ...
      const key = t.displayText || t.clientId;
      const cur = appScopes.get(key) || { name: key, clientId: t.clientId, risky: false, users: [], scopes: new Set(), anonymous: t.anonymous };
      cur.risky = cur.risky || risky;
      cur.users.push(u.primaryEmail);
      scopes.forEach((s) => cur.scopes.add(s));
      appScopes.set(key, cur);
    }
  }
  ...
  if (tokenReadable) {
    ...
    // Concentration risk — many users granting the same broad-scope app
    const highConcentration = [...appScopes.values()].filter((a) => a.risky && a.users.length >= 5).length;
    add({ id: 'g-shadow-concentration', module: 'Risk Center (Shadow IT)', severity: 'medium', cis: 'CIS 2.3',
      title: 'No widely-adopted risky third-party apps', status: highConcentration === 0 ? 'pass' : highConcentration <= 1 ? 'partial' : 'fail',
      detail: `${highConcentration} risky app(s) granted access by 5+ users — indicates informal adoption.`,
      recommendation: 'Investigate broadly-adopted risky apps; if legitimate, formalise via allowlist; otherwise revoke.', fixUrl: `${AC}/owl/list` });

    // Anonymous / uncategorised apps
    const anon = [...appScopes.values()].filter((a) => a.anonymous).length;
    add({ id: 'g-shadow-anon', module: 'Risk Center (Shadow IT)', severity: 'medium', cis: 'CIS 2.3',
      title: 'No anonymous / uncategorised third-party apps', status: anon === 0 ? 'pass' : anon <= 2 ? 'partial' : 'fail',
      detail: `${anon} app(s) reported as anonymous — usually scripts or unverified integrations.`,
      recommendation: 'Review anonymous apps in the Admin console; revoke unfamiliar ones.', fixUrl: `${AC}/owl/list` });
  } else {
  ...
  // Deep email-security analysis per verified domain (live DNS lookups).
  const emailSecurity = {};
  for (const d of domains) {
    try { emailSecurity[d] = await analyseEmailSecurity(d); }
    catch (e) { emailSecurity[d] = { domain: d, error: e.message }; }
  }

  // Activity timeline — last 30 days from the Reports API.
  let activity = null;
  try { activity = await scanActivity(reports); }
  catch (e) { activity = { error: e.message, windowDays: 30, events: [], attention: [], stats: {} }; }
  ...
  return {
    ...
    // Rich data for detail pages
    details: {
      thirdPartyApps: [...appScopes.values()].map((a) => ({
        name: a.name, clientId: a.clientId, risky: a.risky, anonymous: a.anonymous,
        userCount: a.users.length, users: a.users.slice(0, 20),   // cap for payload size
        scopeCount: a.scopes.size, scopes: [...a.scopes].slice(0, 30),
      })).sort((a, b) => (b.risky - a.risky) || (b.userCount - a.userCount)),
      mobileDevices: (mobileRes?.data?.mobiledevices || []).map((d) => ({
        model: d.model || 'Unknown', type: d.type, os: d.os,
        email: (d.email && d.email[0]) || 'unknown',
        compromised: d.deviceCompromisedStatus && d.deviceCompromisedStatus !== 'No compromise detected',
        firstSync: d.firstSync, lastSync: d.lastSync,
      })),
      chromeDevices: (chromeRes?.data?.chromeosdevices || []).map((d) => ({
        model: d.model || 'Unknown', serial: d.serialNumber, status: d.status,
        lastUser: d.recentUsers?.[0]?.email || 'unknown', lastSync: d.lastSync,
      })),
      dormantUsers: stale.slice(0, 50).map((u) => ({
        email: u.primaryEmail, lastLogin: u.lastLoginTime, suspended: false,
      })),
      admins: admins.map((a) => ({
        email: a.primaryEmail, isSuperAdmin: !a.isDelegatedAdmin,
        has2sv: Boolean(a.isEnrolledIn2Sv), enforced2sv: Boolean(a.isEnforcedIn2Sv),
        hasRecovery: Boolean(a.recoveryEmail || a.recoveryPhone),
      })),
      emailSecurity,
      activity,
      // Full user directory for the per-user drill-down page.
      // Cap at 500 to keep the payload sensible; scan.stats.users has the true count.
      users: users.slice(0, 500).map((u) => {
        // Reverse-lookup which OAuth apps this user granted (from our earlier scan)
        const userApps = [...appScopes.values()]
          .filter((a) => a.users.includes(u.primaryEmail))
          .map((a) => ({ name: a.name, risky: a.risky, scopeCount: a.scopes.size }));
        return {
          ...
          isSuperAdmin: Boolean(u.isAdmin && !u.isDelegatedAdmin),
          isDelegatedAdmin: Boolean(u.isDelegatedAdmin),
          has2sv: Boolean(u.isEnrolledIn2Sv),
          enforced2sv: Boolean(u.isEnforcedIn2Sv),
          hasRecovery: Boolean(u.recoveryEmail || u.recoveryPhone),
          recoveryEmail: u.recoveryEmail || null,
          lastLogin: u.lastLoginTime && u.lastLoginTime !== '1970-01-01T00:00:00.000Z' ? u.lastLoginTime : null,
