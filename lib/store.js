...
export async function getTenant(domain) {
  const { rows } = await getPool().query(
    `SELECT domain, name, platform, active, user_count, price_cents, stripe_customer_id, stripe_subscription_id
...
// Marks a tenant active after a confirmed Stripe payment. Idempotent — safe to
// call from both the webhook and the success-page fallback for the same event.
export async function activateTenant(domain, { stripeCustomerId, stripeSubscriptionId } = {}) {
  await getPool().query(
    `UPDATE tenants SET active=true, activated_at=now(),
       stripe_customer_id=COALESCE($2, stripe_customer_id),
       stripe_subscription_id=COALESCE($3, stripe_subscription_id)
     WHERE domain=$1`,
    [domain, stripeCustomerId || null, stripeSubscriptionId || null]);
}
