...
export async function getTenant(domain) {
  const { rows } = await getPool().query(
    `SELECT domain, name, platform, active, user_count, price_cents, payment_customer_id, payment_subscription_id, payment_subscription_token
  ...
  return {
    ...
    paymentSubscriptionToken: r.payment_subscription_token,
  };
}

export async function getTenantByPaymentCustomerId(customerId) {
  const { rows } = await getPool().query(`SELECT domain FROM tenants WHERE payment_customer_id=$1 LIMIT 1`, [customerId]);
  return rows[0] ? { domain: rows[0].domain } : null;
}
...
export async function activateTenant(domain, { paymentCustomerId, paymentSubscriptionId, paymentSubscriptionToken } = {}) {
  await getPool().query(
    `UPDATE tenants SET active=true, activated_at=now(), cancelled_at=NULL,
       payment_customer_id=COALESCE($2, payment_customer_id),
       payment_subscription_id=COALESCE($3, payment_subscription_id),
       payment_subscription_token=COALESCE($4, payment_subscription_token)
     WHERE domain=$1`,
    [domain, paymentCustomerId || null, paymentSubscriptionId || null, paymentSubscriptionToken || null]);
}

// Marks a tenant inactive after a cancelled subscription. Deliberately does NOT
// clear payment_subscription_id/token — Paystack's disable is soft (takes effect
// at the end of the current billing period on their side), and keeping the
// reference means a future re-activation can be traced back to the same subscription.
export async function deactivateTenant(domain) {
  await getPool().query(`UPDATE tenants SET active=false, cancelled_at=now() WHERE domain=$1`, [domain]);
}
